...
class GoodsIPhoneVC: CafeViewController {
    ...
    @IBOutlet weak var checkButton: UIButton! {didSet{checkButton.titleLabel?.lineBreakMode = .byWordWrapping}}
...
extension GoodsIPhoneVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView.tag == 0 {
            let cell:GoodsIPhoneTableViewCell = tableView.dequeueReusableCell(withIdentifier: "Cell") as! GoodsIPhoneTableViewCell
            
            let filtered = goods.filter({$0.group == group[indexPath.section]})
            
            cell.mGoodNameLabel.text = filtered[indexPath.row].coffieName
            cell.mGoodNickNameLabel.text = filtered[indexPath.row].nickName
//            cell.textLabel?.text = filtered[indexPath.row].coffieName + " / " + filtered[indexPath.row].nickName
//            cell.detailTextLabel?.text = filtered[indexPath.row].nickName
    ...
    func updateBill() {
        ...
        checkButton.setTitle("結帳\n\(bill)", for: .normal)
    }
}

class GoodsIPhoneTableViewCell: UITableViewCell {
    
    @IBOutlet weak var mGoodNameLabel: UILabel!
    @IBOutlet weak var mGoodNickNameLabel: UILabel!
